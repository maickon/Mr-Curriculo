...
$textAreaLabel = array('Áreas');
$textAreaText = array('area');


$pagina_nome = 'Interesses. Aqui você informa quais são suas áreas de interesse.';
$descriacao= array('create'=>'Cadastrando um novo interesse.',
				   'edit'=>'Editando a um interesse.',
				   'destroy'=>'Apagando informações de interesse.');
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Descrição das áreas de interesse','Ações');
			 		ths($labels);
			 	$tag->close('tr');
