...
$textAreaLabel = array('Objetivo');
$textAreaText = array('texto');


$pagina_nome = 'Objetivo. Aqui você informa quais são seus objetivos profissionais.';
$descriacao= array('create'=>'Cadastrando um novo objetivo.',
				   'edit'=>'Editando um objetivo.',
				   'destroy'=>'Apagando um objetivo.');
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Descrição do seu objetivo','Ações');
			 		ths($labels);
			 	$tag->close('tr');
