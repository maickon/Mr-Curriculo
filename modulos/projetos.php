...
$inputLabel = array('Título','Linguagem','Link','Ano');
$inputName = array('titulo','linguagem','link','ano');

$textAreaLabel = array('Descricao');
$textAreaText = array('texto');

$pagina_nome = 'Projetos. Aqui você informa sobre todos os seus projetos de software que você desenvolveu.';
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Nome do projeto','Linguagem utilizada','Ano','Link de referência','Descrição','Ações');
			 		ths($labels);
			 	$tag->close('tr');
