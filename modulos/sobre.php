...
$textAreaLabel = array('Sobre o site MR Currículo');
$textAreaText = array('texto');


$pagina_nome = 'Sobre. Aqui você informa sobre a razão pela criação do seu próprio site.';
$descriacao= array('create'=>'Cadastrando a página sobre.',
				   'edit'=>'Editando a página sobre.',
				   'destroy'=>'Apagando a página sobre.');
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Descrição da página sobre','Ações');
			 		ths($labels);
			 	$tag->close('tr');
