...
$textAreaLabel = array('Apresentação');
$textAreaText = array('texto');


$pagina_nome = 'Apresentação. Aqui você vai falar um pouco sobre você.';
$descriacao= array('create'=>'Cadastrando uma apresentação.',
				   'edit'=>'Editando uma apresentação.',
				   'destroy'=>'Apagando uma apresentação.');
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Descrição da sua apresentação','Ações');
			 		ths($labels);
			 	$tag->close('tr');
